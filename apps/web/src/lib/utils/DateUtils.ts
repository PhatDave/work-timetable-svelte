...
export function get_first_day_in_month(month: number, year: number): DayEnumItem {
    month -= 1;
...
export function get_last_n_days(month: number, year: number, n: number): Date[] {
    const days: Date[] = [];
...
export function get_first_n_days(month: number, year: number, n: number): Date[] {
    const days: Date[] = [];
...
export function build_month(month: number, year: number): Day[] {
    ...
    const num_next_month_days: number = MONTH_DAYS - num_previous_month_days - get_days_in_month(month, year);

    const previous_month_days: Date[] = get_last_n_days(month - 1, year, num_previous_month_days);
    const next_month_days: Date[] = get_first_n_days(month + 1, year, num_next_month_days);
