...
export async function get_or_create_user(username: string) {
    ...
    let res = await fetch(`${API_COLLECTIONS_URL}/user/records?filter=(name='${username}')`);
...
export async function persist_day(day: Day) {
    ...
    let res = await fetch(`${API_COLLECTIONS_URL}/day/records?filter=(name~'${day.date}')`);
    const json: APIResponse = await res.json();
...
export async function create_worktime(day: Day, hours: number) {
    ...
    const res = await fetch(`${API_COLLECTIONS_URL}/work_time/records`, post_options);
    if (res.status === 200) {
...
export async function delete_worktime_api(worktime: Worktime) {
    ...
    return await fetch(`${API_COLLECTIONS_URL}/work_time/records/${worktime.id}`, delete_options);
}


export async function create_overtime(day: Day, hours: number, description: string) {
    const user_val = get(user);
    if (!user_val) {
        return;
    }
...
export async function delete_overtime_api(worktime: Worktime) {
    ...
    return await fetch(`${API_COLLECTIONS_URL}/overtime/records/${worktime.id}`, delete_options);
}
