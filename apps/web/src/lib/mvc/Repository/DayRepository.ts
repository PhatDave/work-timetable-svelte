...
export default class DayRepository {
    private readonly day_api_url: string = "https://pocketbase-work-timetable.site.quack-lab.dev/api/collections/day/records";
    private readonly default_page_size: string = "?perPage=100";
    private readonly default_expand: string = "&expand=work_time(day),overtime(day)";
    private readonly default_fields: string = "&fields=id,name";
    private readonly default_sort: string = "&sort=name";
    private readonly default_filter: string = "&filter=(name >= '{1}')&&(name <= '{2}')";
    ...
    public async get_all_days_full(start: Date, end: Date): Promise<APIResponseDTO> {
    ...
    private construct_default_url(start: Date, end: Date): string {
        ...
        url += this.default_filter.replace("{1}", start.toISOString()).replace("{2}", end.toISOString());
        url += this.default_expand;
