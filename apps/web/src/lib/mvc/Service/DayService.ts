...
export default class DayService {
    ...
    public async get_this_month_days_full(): Promise<Day[]> {
        ...
        const this_month: Day[] = build_month(now.getMonth() + 1, now.getFullYear());
        ...
        const api_days: Day[] = await this.get_all_days_full(first_day, last_day);
        for (let i = 0; i < api_days.length; i++) {
            const api_day: Day = api_days[i];
            const existing_day: Day | undefined = this_month.find((day: Day) => day.date === api_day.date);
            if (existing_day) {
    ...
    public async get_all_days(start: Date, end: Date): Promise<Day[]> {
        const api_data: APIResponseDTO = await this.repository.get_all_days(start, end);
        return this.mapper.to_entity(api_data);
    }

    public async get_all_days_full(start: Date, end: Date): Promise<Day[]> {
        const api_data: APIResponseDTO = await this.repository.get_all_days_full(start, end);
        return this.mapper.to_entity(api_data);
    }
